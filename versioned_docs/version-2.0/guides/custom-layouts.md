...
Layouts can read properties from `p.tag` to let users control behavior at runtime. The most common are `master_count` and `master_width_factor`. See [Master and Stack](/docs/concepts/master-and-stack) for what each property does.
...
- **[Layout Protocol Reference](/docs/reference/awful/layout)** - Full `arrange(p)` contract, all fields, built-in layouts
- **[Master and Stack](/docs/concepts/master-and-stack)** - How tag properties control primary/secondary splits
