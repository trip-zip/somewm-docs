...
When focus moves to a column outside the viewport, the carousel computes a new `target_offset` and animates toward it. The exact behavior depends on the [centering mode](/docs/reference/awful/carousel#centering-modes):
...
- **[Carousel Tutorial](/docs/tutorials/carousel)** - Step-by-step first session
- **[Master and Stack](/docs/concepts/master-and-stack)** - How traditional tiling layouts divide the screen
- **[Layout Protocol Reference](/docs/reference/awful/layout)** - The `arrange(p)` contract
