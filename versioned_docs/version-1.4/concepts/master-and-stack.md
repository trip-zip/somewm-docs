...
The compositor maintains a list of all clients in creation order (see [Client Stacking](/docs/concepts/client-stack#the-two-client-arrays) for how this differs from visual z-order). When a layout runs, it reads this list and assigns windows to sections:
...
| `carousel` | No | Scrollable columns with independent widths ([details](/docs/concepts/scrollable-tiling)) |
| `carousel.vertical` | No | Vertical variant of carousel ([details](/docs/concepts/scrollable-tiling)) |
...
- [Layout Protocol Reference](/docs/reference/awful/layout) - The `arrange(p)` contract and built-in layouts
- [Custom Layouts Guide](/docs/guides/custom-layouts) - Write your own tiling layout from scratch
- [Client Stacking](/docs/concepts/client-stack) - Z-order and visual overlap (different from layout order)
- [Client Rules](/docs/guides/client-rules) - Automatically set properties when windows open
- [Focus History](/docs/concepts/focus-history) - What determines focus when a client closes
